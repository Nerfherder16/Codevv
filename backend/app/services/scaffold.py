...
async def run_scaffold_job(job_id: uuid.UUID, db: AsyncSession):
    result = await db.execute(select(ScaffoldJob).where(ScaffoldJob.id == job_id))
    job = result.scalar_one_or_none()
    if not job:
        return

    job.status = ScaffoldStatus.generating
    await db.flush()
    await db.commit()

    try:
        # Fetch components
        component_ids = [uuid.UUID(cid) for cid in job.component_ids]
  ...
  "components": [
    {{
      ...
      "api_url": "/api/items"
        ...
        # Generate files from spec using Jinja2
        generated_files = {}
        ...
        job.status = ScaffoldStatus.review
        job.completed_at = datetime.now(timezone.utc)

    except Exception as e:
        job.status = ScaffoldStatus.failed
